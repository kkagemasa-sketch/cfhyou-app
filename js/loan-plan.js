  ...
  for(let y=1;y<=50&&(balM+balB+unpaidIntM+unpaidIntB)>0;y++){
    ...
    if(fiveYearRule){
      ...
      if(curMR!==mr){
        mr=curMR;
        halfRate=curHalfRate;
        // 月額を「もし通常通り元の期間で再計算したらいくらか」を計算
        const remM=Math.max(1,(years-(y-1))*12);
        const newMpMonthly = mr>0 ? balM*mr*Math.pow(1+mr,remM)/(Math.pow(1+mr,remM)-1) : balM/remM;
        const remH=Math.max(1,(years-(y-1))*2);
        const newMpBonus = halfRate>0 ? balB*halfRate*Math.pow(1+halfRate,remH)/(Math.pow(1+halfRate,remH)-1) : balB/remH;
        // ★ 期間短縮型で過去に繰上返済があれば、月額を「下げないが上げることはある」方針:
        //   - 金利低下: 新月額 < 現月額 → 据え置き（短縮効果を維持）
        //   - 金利上昇: 新月額 > 現月額 → 新月額に上げる（月額不足で期間が延びるバグを防止）
        //   この方針により「期間短縮型なのに金利上昇で期間が延びる」「金利低下で短縮効果が消える」
        //   どちらの逆効果も防げる。
...
function renderLoanTab(){
  ...
  rb.innerHTML=h;
  if(pairLoanMode) togglePairLoan(true);
  // 保存データの復元
  ...
  if(pairLoanMode){const taxB=$('lp-ded-tax-b');if(taxB)taxB.style.display='grid';}
  updateLPDedHint();
  renderLoanCalc();
}
function togglePairLoan(on){
  ...
  if(typeof setLoanMode==='function'){
    ...
    $('loan-pair-tab')?.classList.toggle('on',on);
    $('loan-single-body').style.display=on?'none':'';
    $('loan-pair-body').style.display=on?'':'none';
  }
  renderLoanCalc();
}
...
function renderLoanCalc(){
  ...
  if(_lctrlDedMode==='manual'){
    ...
    for(let y=0;y<dedYrs;y++){
      ...
      let rawB=pairLoanMode?Math.round(Math.min(balBEnd,dedLimit)*dedRate):0;
      ...
      let dB=(pairLoanMode&&hasTaxLimitW)?Math.round(itPartB+jtPartB):rawB;
  ...
  if(showPair){
    ...
    t+=`<tr style="background:var(--navy);color:#fff">
      ...
      <th style="${thS}">月額</th>${_showBonusCol?`<th style="${thS};background:#7a5000">ボーナス(1回)</th><th style="${thS};background:#7a5000">ボーナス年合計</th>`:''}<th style="${thS}">返済合計</th><th style="${thS}">元金</th><th style="${thS}">利息</th><th style="${thS}">繰上返済</th><th style="${thS}">元金残高</th>
  ...
  for(let i=0;i<rowCount;i++){
    ...
    if(showPair){
      ...
      const bonusCells = _showBonusCol
        ? `<td style="${td};color:#b45309;background:${i%2===0?'#fffbeb':'#fef3c7'}">${rA&&rA.bonusPay>0?Math.round(rA.bonusPay).toLocaleString():'-'}</td><td style="${td};color:#b45309;background:${i%2===0?'#fffbeb':'#fef3c7'}">${rA&&rA.bonusPay>0?Math.round(rA.bonusPay*2).toLocaleString():'-'}</td>`
        : '';
  ...
  if(showPair){
    ...
    const totBonus = _showBonusCol ? dataA.reduce((s,r)=>s+(r.bonusPay||0)*2,0) : 0;
    const bonusTotalCells = _showBonusCol
      ? `<td style="${td};background:#7a5000;color:#fff">-</td><td style="${td};background:#7a5000;color:#fff">${totBonus>0?Math.round(totBonus).toLocaleString():'-'}</td>`
      : '';
    t+=`<tr style="background:var(--navy);color:#fff;font-weight:700">
