...
const EDU_TABLE = {
  ...
  // ===== 大学院 =====
  // master: 修士課程（2年）
  // doctor: 博士課程（3年）— 修士修了後に進学
  grad:{
    master:{
      // 1年目=学費+入学金+生活費、2年目=学費+生活費（入学金なし）
      nat_h:  [109, 81],
      nat_b:  [220, 192],
      plit_h: [156, 108],
      plit_b: [295, 247],
    ...
    doctor:{
      // 3年間 すべて学費+生活費（入学金は再度発生する想定で初年度+10万）
      nat_h:  [91, 81, 81],
      nat_b:  [202, 192, 192],
      plit_h: [118, 108, 108],
      plit_b: [257, 247, 247],
...
// ===== 教育費3区分内訳（文部科学省「子供の学習費調査」準拠） =====
// 学校教育費 / 学校給食費 / 学校外活動費 の3区分
// 各配列は合計が EDU_TABLE の対応値とほぼ一致するように配分
// 大学は「学費+入学金/施設費+生活費」の3区分に意味を変えている
const EDU_BD = {
  elem:{
    // 学校教育費（授業料・教材・制服・通学費・PTA等）
    eduFee:{
      public: [0,0, 10, 5, 5, 5, 5, 8],      // 1年は入学準備費、6年は修学旅行で高め
      private:[0,0,157,99,99,100,99,128]
    },
    // 学校給食費
    lunch:{
      public: [0,0, 4, 4, 4, 4, 4, 4],
      private:[0,0, 4, 4, 4, 4, 4, 4]
    },
    // 学校外活動費（塾・習い事・家庭学習教材）
    extra:{
      public: [0,0, 25, 22, 22, 24, 23, 24],
      private:[0,0, 59, 65, 65, 68, 66, 69]
    }
  },
  mid:{
    eduFee:{ public:[15,12,12], private:[116,102,102] },
    lunch:{  public:[ 4, 4, 4], private:[  1,  1,  1] },
    extra:{  public:[38,37,37], private:[ 48, 48, 48] }
  },
  high:{
    // 高校は給食なし
    eduFee:{ public:[40,27,25], private:[82,72,71] },
    lunch:{  public:[ 0, 0, 0], private:[ 0, 0, 0] },
    extra:{  public:[30,30,27], private:[28,28,27] }
  },
  // 大学: 学費 / 入学金+施設費 / 生活費（自宅/下宿で異なる）
  univ:{
    tuition:{
      nat_h:[54,54,54,54], nat_b:[54,54,54,54],
      plit_h:[81,81,81,81], plit_b:[81,81,81,81],
      psci_h:[114,114,114,114], psci_b:[114,114,114,114],
      med_h:[286,286,286,286,286,286], med_b:[286,286,286,286,286,286],
      senmon_h:[80,80], senmon_b:[80,80],
      none:[]
    },
    entrance:{ // 入学金(初年度)＋施設費(毎年)
      nat_h:[28,0,0,0], nat_b:[28,0,0,0],
      plit_h:[40,15,15,15], plit_b:[40,15,15,15],
      psci_h:[43,18,18,18], psci_b:[43,18,18,18],
      med_h:[232,88,88,88,88,88], med_b:[232,88,88,88,88,88],
      senmon_h:[20,15], senmon_b:[20,15],
      none:[]
    },
    living:{ // 生活費（自宅 or 下宿）
      nat_h:[93,58,58,58], nat_b:[208,110,110,110],
      plit_h:[105,70,70,70], plit_b:[234,129,129,129],
      psci_h:[104,67,67,67], psci_b:[232,125,125,125],
      med_h:[65,71,71,71,71,71], med_b:[193,129,129,129,129,129],
      senmon_h:[50,35], senmon_b:[160,105],
      none:[]
    }
  },
  // 保育: 保育料のみ（3区分ではなく単一カテゴリ）
  hoiku:{ label:'保育料' }
};
...
const _STATIC_FIELDS=['client-name','husband-age','wife-age','h-death-age','w-death-age','izoku-mode',
  'h-income-mode','w-income-mode','h-work-type','w-work-type',
...
const FLAT_RATE_TABLE=[
  ['2024-01',1.87,1.39,2.23],['2024-02',1.82,1.34,2.22],['2024-03',1.84,1.36,2.20],
  ['2024-04',1.82,1.43,1.92],['2024-05',1.83,1.44,1.93],['2024-06',1.85,1.46,1.95],
  ['2024-07',1.84,1.45,1.94],['2024-08',1.85,1.46,1.95],['2024-09',1.82,1.43,1.92],
  ['2024-10',1.82,1.43,1.92],['2024-11',1.84,1.45,1.94],['2024-12',1.86,1.47,1.96],
  ['2025-01',1.86,1.47,1.96],['2025-02',1.89,1.50,1.99],['2025-03',1.94,1.55,2.03],
  ['2025-04',1.94,1.55,2.04],['2025-05',1.82,1.43,1.92],['2025-06',1.89,1.50,1.99],
  ['2025-07',1.84,1.45,1.94],['2025-08',1.87,1.48,1.97],['2025-09',1.89,1.50,1.99],
  ['2025-10',1.89,1.50,1.90],['2025-11',1.90,1.51,2.00],['2025-12',1.97,1.58,2.07],
  ['2026-01',2.08,1.71,2.02],['2026-02',2.26,1.91,2.38],['2026-03',2.25,1.92,2.38],
  ['2026-04',2.49,2.17,2.64],['2026-05',2.71,2.39,2.87],['2026-06',3.21,2.89,3.38],
  ['2026-07',3.14,2.82,3.32],['2026-08',3.29,2.97,3.50],['2026-09',3.46,3.14,3.70],
];
