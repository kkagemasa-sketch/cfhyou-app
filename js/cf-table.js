...
function renderTable(R,total,disp,cLbls,cYear,loanAmt,isM,hAge,retAge,children,delivery){
  ...
  (()=>{
    const _anyOwner=['carTotalH','carTotalW','carTotalS'].some(k=>R[k]&&R[k].slice(0,disp).some(v=>v>0));
    // [rowKey, ラベル, 値配列, 内訳フィルタ(owner)]
    const _carRowDefs=[
      ['carTotal', '車両費・車検', _anyOwner?R.carTotalNone:R.carTotal, _anyOwner?'none':null],
      ['carTotalH','車両費・車検（ご主人様）', R.carTotalH, 'h'],
      ['carTotalW','車両費・車検（奥様）', R.carTotalW, 'w'],
      ['carTotalS','車両費・車検（共用）', R.carTotalS, 'share'],
    ];
    _carRowDefs.forEach(([rowKey,lbl,arr,ownerFilter])=>{
      if(!arr)return;
      const dl=_rl(rowKey,lbl);
      let tot=0;
      ...
      for(let i=0;i<disp;i++){
        ...
        // 内訳ツールチップ（この行の所有者の車だけ表示）
        const bdAll=(R.carBd&&R.carBd[i])||[];
        const bdItems=ownerFilter===null?bdAll:bdAll.filter(it=>(it.owner||'none')===ownerFilter);
        let title='';
  ...
  const _initSavV=ri(window._purchaseInitSav||0);const _initSavTxt=_initSavV>=0?_initSavV.toLocaleString():'▲'+Math.abs(_initSavV).toLocaleString();const _initSavStyle=_initSavV<0?'background:#ffe3e3;color:#a3000c;padding:0 5px;border-radius:3px':'';
  // 預貯金マイナスの年を検出（警告用）
  const _negYears=[];
  let _minSav=0;
  for(let i=0;i<disp;i++){
  ...
  if(R.finAsset.some(v=>v>0)){
    ...
    ['h','w'].forEach(p=>{
      ...
      document.querySelectorAll(`[id^="sec-bal-${p}-"],[id^="sec-stk-bal-${p}-"]`).forEach(el=>{
        ...
        if(isAcc){
          val=fv(`sec-bal-${p}-${sid}`)||0;
          lbl=secRowLabel(p,sid,'課税積立');
        } else if(isStock){
          val=fv(`sec-stk-bal-${p}-${sid}`)||0;
          lbl=secRowLabel(p,sid,'課税一括投資');
        } else return;
        if(val<=0)return;
